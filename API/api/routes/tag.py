...
post_parser.add_argument('Authorization', type=str, required=True, help='Bearer token', 
                         location='headers')

get_parser = reqparse.RequestParser()
get_parser.add_argument('note_id', type=int, required=True, help='Note ID')
get_parser.add_argument('Authorization', type=str, required=True, help='Bearer token', 
                        location='headers')
...
delete_parser.add_argument('Authorization', type=str, required=True, 
                           help='Bearer token', location='headers')
...
put_parser.add_argument('Authorization', type=str, required=True, help='Bearer token', 
                        location='headers')


@api.route("/note/tags")
@api.doc(description=
'Endpoint for creating, updating, deleting and fetching tags for a note.')
class Tag(Resource):
    ...
    def post(self):
        ...
        if not note_id or not tag_name:
            return {"message": "Invalid request body. Ensure that note_id and tag_name "
                    "are present and in the correct format."}, 400
        ...
        if not user_has_permission(current_user, note_id):
            return {"message": 
                    "You do not have permission to create tags for this note."}, 403
        
        existing_tag = TagService.get_existing_tag(note_id, tag_name)
        if existing_tag:
            return {"message": f"Tag '{tag_name}' already exists for note {note_id}"
                    }, 400

        try:
            new_tag = TagService.create_tag(note_id, tag_name)
        except Exception:
            return {"message": "An error occurred while creating the tag. "
                    "Please try again."}, 500

        return {"message": f"Tag '{tag_name}' has been successfully created.", 
                "tag_id": new_tag.tag_id}, 201
    ...
    def get(self):
        ...
        if not note_id:
            return {"message": "Invalid request. Ensure that note_id is present and in"
                    " the correct format."}, 400
        ...
        if not user_has_permission(current_user, note_id):
            return {"message": 
                    "You do not have permission to view tags for this note."}, 403
    ...
    def delete(self):
        ...
        if not tag_id:
            return {"message": "Invalid request body. Ensure that tag_id is present and"
                    " in the correct format."}, 400
        ...
        if not user_has_permission(current_user, note_tag.note_id):
            return {"message": 
                    "You do not have permission to delete tags for this note."}, 403
    ...
    def put(self):
        ...
        return {"tag_id": note_tag.tag_id, "note_id": note_tag.note_id, 
                "tag_name": tag.name}, 200
