...
def test_post(client):
    ...
    response = client.post(
        f'/note/tags?note_id={note_id}&tag_name={tag_name}',
...
def test_put_note_tags_400(client):
    ...
    assert response.json == {
            "message": "Input payload validation failed",
            "errors": {
                "tag_id": "Tag ID Missing required parameter in the JSON body or the "
                "post body or the query string"
            }
        }
...
def test_put_note_tags_403(client):
    ...
    response = client.put(
        f'/note/tags?tag_id={tag_id}&tag_name={new_tag_name}',
        headers={'Authorization': f'Bearer {token}'}
    )
